...
expect()->extend('toHaveContentsMatching', function (string $contents) {
    expect($this->value)->toBeFile()
        ->and(file_get_contents($this->value))->toBe($contents);
...
expect()->extend('toHaveContentsIncluding', function (string $contents) {
    expect($this->value)->toBeFile()
        ->and(file_get_contents($this->value))->toContain($contents);
...
expect()->extend('toHaveContentsMatchingFile', function (string $filename) {
    expect($this->value)->toBeFile()
        ->and($filename)->toBeFile()
        ->and(file_get_contents($this->value))->toBe(file_get_contents($filename));
...
expect()->extend('toHaveOnlyExpectedMigrationsLoaded', function (...$expectedFiles) {
    ...
    foreach ($migrationFiles as $migrationFile) {
        ...
        if (in_array($migrationFile, $expectedFiles, true)) {
            continue;
        }
...
function getLoadedMigrations(string $vendorMigrationPath): array
{
    ...
    return collect(app('migrator')->paths())
        ->map(fn (string $file): string => Str::replace('\\', '/', Str::after(realpath($file), $vendorMigrationPath)))
        ->toArray();
}
...
function getDirectoryContents(string $path): array
{
    ...
    return collect(File::allFiles($path))
        ->map(fn (SplFileInfo $file): string => Str::replace('\\', '/', Str::after(realpath($file->getPathname()), $path)))
        ->toArray();
}
...
function isFileListed(array $listedFiles, string $expectedFile, bool $endsWith): bool
{
    ...
    $filePath = mb_substr($expectedFile, 0, -mb_strlen($fileName) - 1);

    return arrayAny($listedFiles, static function (string $file, int $ix) use ($filePath, $fileName, $endsWith) {
        $fileBase = basename($file);
        if (str_contains($file, '/') && ! str_starts_with($file, $filePath)) {
            return false;
        }
