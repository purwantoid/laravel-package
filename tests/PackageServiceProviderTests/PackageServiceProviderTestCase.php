...
namespace Purwantoid\LaravelPackage\Tests\PackageServiceProviderTests;
...
use Purwantoid\LaravelPackage\Tests\TestCase;
use Purwantoid\LaravelPackage\Tests\TestPackage\Src\TestServiceProvider;
...
abstract class PackageServiceProviderTestCase extends TestCase
{
    ...
    protected function deletePublishedDirectoriesPartial(): self
    {
        ...
        foreach ($this->cleanPathsPartial as $dir) {
            ...
            collect(File::allFiles($dir))->each(function (SplFileInfo $file) use ($basePath): void {
                if (! in_array(Str::replace('\\', '/', Str::after($file->getPathname(), $basePath)), $this->cleanExclusions, true) && ! unlink($file->getPathname())) {
